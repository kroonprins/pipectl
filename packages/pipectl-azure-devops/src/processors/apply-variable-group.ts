...
class ApplyVariableGroup implements ActionProcessor {
  ...
  canProcess(transformedDefinition: TransformedDefinition, action: Action, _args: ApplyArguments): boolean {
    return transformedDefinition instanceof AzureVariableGroup && action === Action.APPLY
  }
  ...
  async process(azureVariableGroup: AzureVariableGroup, _action: Action, args: ApplyArguments): Promise<ProcessResult> {
    ...
    if (existingVariableGroup) {
      ...
      if (args.dryRun) {
        return { info: `Variable group ${variableGroup.name} update skipped because dry run.` }
      } else {
        try {
          await variableGroupApi.updateVariableGroup(variableGroup, project)
          return { info: `Successfully updated variable group ${existingVariableGroup.id} (${existingVariableGroup.name})` }
        } catch (e) {
          return { error: e }
        }
      }
      ...
      if (args.dryRun) {
        return { info: `Variable group ${variableGroup.name} creation skipped because dry run.` }
      } else {
        try {
          const createdVariableGroup = await variableGroupApi.createVariableGroup(variableGroup, project)
          return { info: `Successfully created variable group ${createdVariableGroup.id} (${createdVariableGroup.name})` }
        } catch (e) {
          return { error: e }
        }
      }
