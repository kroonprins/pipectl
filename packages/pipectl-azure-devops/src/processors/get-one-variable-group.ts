...
class GetOneVariableGroup implements ActionProcessor {
  ...
  async process(azureVariableGroup: AzureVariableGroup, _action: Action, args: GetArguments): Promise<GetVariableGroupProcessResult> {
    log.debug(`[GetOneVariableGroup] ${JSON.stringify(azureVariableGroup)}`)
    try {
      const project = azureVariableGroup.project
      const variableGroup = await variableGroupApi.findVariableGroupById(Number(args.name), project)
      if (variableGroup) {
        return new GetVariableGroupProcessResult([variableGroup])
      } else {
        return { error: new Error(`Variable group '${args.name}' does not exist in project '${project}'.`) }
      }
    } catch (e) {
      return { error: e }
    }
