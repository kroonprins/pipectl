...
class VariableGroupApi {
  ...
  async findVariableGroupByName(name: string, project: string): Promise<VariableGroup | null> {
    log.debug(`[VariableGroupApi.findVariableGroupByName] name[${name}], project[${project}]`)
    const api = await this.getApi()
    const search = await api.getVariableGroups(project, name)
    if (search && search.length) {
      log.debug(`[VariableGroupApi.findVariableGroupByName] found ${search[0].id} (${search.length}) for name[${name}], project[${project}]`)
      return search[0]
    }
    log.debug(`[VariableGroupApi.findVariableGroupByName] not found for name[${name}], project[${project}]`)
    return null
  }

  async findVariableGroupById(id: number, project: string) {
    log.debug(`[VariableGroupApi.findVariableGroupById] id[${id}], project[${project}]`)
    const api = await this.getApi()
    ...
    throw new Error(`Variable group with id ${id} not found in project ${project}. It either doesn't exist or you do not have the required access for it.`)
  }
  ...
  async createVariableGroup(variableGroup: VariableGroup, project: string) {
    log.debug(`[VariableGroupApi.createVariableGroup] ${variableGroup.name}, project[${project}]`)
    const api = await this.getApi()
    return api.addVariableGroup(variableGroup)
  }

  async updateVariableGroup(variableGroup: VariableGroup, project: string) {
    log.debug(`[VariableGroupApi.updateVariableGroup] ${variableGroup.name}, project[${project}]`)
    const api = await this.getApi()
    return api.updateVariableGroup(variableGroup, variableGroup.id!)
  }

  async deleteVariableGroup(id: number, project: string) {
    log.debug(`[VariableGroupApi.deleteVariableGroup] id[${id}], project[${project}]`)
    const api = await this.getApi()
