import { AgentPoolQueue, AgentPoolQueueTarget, AgentTargetExecutionOptions, BuildAuthorizationScope, BuildDefinition, BuildDefinitionStep, BuildDefinitionVariable, BuildProcess, BuildRepository, BuildTrigger, ContinuousIntegrationTrigger, DefinitionQuality, DefinitionReference, DefinitionTriggerType, DefinitionType, DesignerProcess, Phase, PhaseTarget, Schedule, ScheduleDays, ScheduleTrigger } from 'azure-devops-node-api/interfaces/BuildInterfaces'
import { applyExport, filterProp } from './export'

const drafts = async (buildDefinition: BuildDefinition): Promise<DefinitionReference[] | undefined> => {
  return (buildDefinition.drafts || []).length !== 0 ? buildDefinition.drafts : undefined
}

const process = async (buildDefinition: BuildDefinition): Promise<BuildProcess> => {
  const commonExportApplied = await applyExport(buildDefinition.process || {}, exportProcess)
...
const phaseTarget = async (phase: Phase): Promise<PhaseTarget | undefined> => {
  let target = await applyExport(phase.target || {}, exportDesignerProcessPhaseTarget)
  if (phase.target) {
    if (phase.target.type === 1) { // AgentPoolQueueTarget
      target = await applyExport(target, exportDesignerProcessPhaseAgentPoolQueueTarget)
    } // TODO other target types?
  }
  return Object.keys(target).length !== 0 ? target : undefined
}

const designerProcessPhaseAgentPoolQueueTargetExecutionOptions = async (agentPoolQueueTarget: AgentPoolQueueTarget): Promise<AgentTargetExecutionOptions | undefined> => {
  const executionOptions = applyExport(agentPoolQueueTarget.executionOptions || {}, exportDesignerProcessPhaseAgentPoolQueueTargetExecutionOptions)
  return Object.keys(executionOptions).length !== 0 ? executionOptions : undefined
}

const phaseSteps = async (phase: Phase): Promise<BuildDefinitionStep[]> => {
  return Promise.all(
    (phase.steps || [])
      .map(step => applyExport(step, exportDesignerProcessStep))
  )
}

const designerProcessStepEnvironment = async (buildDefinitionStep: BuildDefinitionStep): Promise<{ [key: string]: string } | undefined> => {
  return Object.keys(buildDefinitionStep.environment || {}).length !== 0 ? buildDefinitionStep.environment : undefined
}

const properties = async (buildDefinition: BuildDefinition): Promise<any> => {
  return Object.keys(buildDefinition.properties).length !== 0 ? buildDefinition.properties : undefined
}

const queue = async (buildDefinition: BuildDefinition): Promise<AgentPoolQueue | undefined> => {
  return buildDefinition.queue ? applyExport(buildDefinition.queue, exportQueue) : undefined
}

const repository = async (buildDefinition: BuildDefinition): Promise<BuildRepository> => {
  return applyExport(buildDefinition.repository || {}, exportBuildRepository)
}

const buildRepositoryProperties = async (buildRepository: BuildRepository): Promise<{ [key: string]: string; } | undefined> => {
  const exportedProperties = applyExport(buildRepository.properties || {}, exportBuildRepositoryProperties)
  return Object.keys(exportedProperties).length !== 0 ? exportedProperties : undefined
}

const tags = async (buildDefinition: BuildDefinition): Promise<any> => {
  return (buildDefinition.tags || []).length !== 0 ? buildDefinition.tags : undefined
}

const triggers = async (buildDefinition: BuildDefinition): Promise<BuildTrigger[]> => {
  return Promise.all(
...
const schedules = async (scheduleTrigger: ScheduleTrigger): Promise<Schedule[]> => {
  return Promise.all(
    (scheduleTrigger.schedules || [])
      .map(schedule => applyExport(schedule, exportSchedule))
  )
}

const variableGroups = async (buildDefinition: BuildDefinition): Promise<string[] | undefined> => {
  const vars = (buildDefinition.variableGroups || []).map(v => v.name || v.id!.toString())
  return vars.length !== 0 ? vars : undefined
}

const variables = async (buildDefinition: BuildDefinition): Promise<{ [key: string]: BuildDefinitionVariable } | { [key: string]: string | undefined } | undefined> => {
  const vars =
    (await Promise.all(
      Object.entries(buildDefinition.variables || {})
...
const exportDesignerProcessPhaseAgentPoolQueueTarget: AgentPoolQueueTarget | object = {
