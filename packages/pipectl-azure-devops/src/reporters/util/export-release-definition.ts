import { AgentBasedDeployPhase, AgentDeploymentInput, ApprovalOptions, Artifact, ArtifactDownloadInputBase, ArtifactsDownloadInput, ArtifactSourceReference, Condition, ConfigurationVariableValue, Demand, DeployPhase, DeployPhaseTypes, EnvironmentExecutionPolicy, EnvironmentOptions, EnvironmentRetentionPolicy, EnvironmentTrigger, ExecutionInput, ReleaseDefinition, ReleaseDefinitionApprovals, ReleaseDefinitionApprovalStep, ReleaseDefinitionEnvironment, ReleaseDefinitionGate, ReleaseDefinitionGatesStep, ReleaseSchedule, ReleaseTriggerBase, WorkflowTask } from 'azure-devops-node-api/interfaces/ReleaseInterfaces'
...
const conditions = async (environment: ReleaseDefinitionEnvironment): Promise<Condition[] | undefined> => {
  return (environment.conditions || []).length !== 0 ? environment.conditions : undefined
}

const demands = async (environment: ReleaseDefinitionEnvironment | AgentDeploymentInput): Promise<Demand[] | undefined> => {
  return (environment.demands || []).length !== 0 ? environment.demands : undefined
}

const deployPhases = async (environment: ReleaseDefinitionEnvironment, projectId: string): Promise<DeployPhase[] | undefined> => {
  const exported = await Promise.all(
    (environment.deployPhases || [])
      .map(async deployPhase => {
        const exportedDeployPhase = await applyExport(deployPhase, exportDeployPhase)
        if (deployPhase.phaseType === DeployPhaseTypes.AgentBasedDeployment) {
          return applyExport(exportedDeployPhase, exportAgentBasedDeployPhase, projectId)
        }
...
const environmentOptions = async (environment: ReleaseDefinitionEnvironment): Promise<EnvironmentOptions | undefined> => {
  const exported = await applyExport(environment.environmentOptions || {}, exportEnvironmentOptions)
  return Object.keys(exported).length !== 0 ? exported : undefined
}

const environmentTriggers = async (environment: ReleaseDefinitionEnvironment): Promise<EnvironmentTrigger[] | undefined> => {
  return (environment.environmentTriggers || []).length !== 0 ? environment.environmentTriggers : undefined
}

const executionPolicy = async (environment: ReleaseDefinitionEnvironment): Promise<EnvironmentExecutionPolicy | undefined> => {
  const exported = await applyExport(environment.executionPolicy || {}, exportExecutionPolicy)
  return Object.keys(exported).length !== 0 ? exported : undefined
}

const workflowTasks = async (deployPhase: DeployPhase): Promise<WorkflowTask[] | undefined> => {
  const exported = (await Promise.all(
    (deployPhase.workflowTasks || [])
      .map(workflowTask => {
        return applyExport(workflowTask || {}, exportWorkflowTask)
      })
  )).filter(workflowTask => workflowTask && Object.keys(workflowTask).length !== 0)
  return exported.length !== 0 ? exported : undefined
}

const deploymentInput = async (deployPhase: AgentBasedDeployPhase, projectId: string): Promise<AgentDeploymentInput | undefined> => {
  const exported = await applyExport(deployPhase.deploymentInput || {}, exportAgentDeploymentInput, projectId)
  return Object.keys(exported).length !== 0 ? exported : undefined
}

const queueId = async (agentDeploymentInput: AgentDeploymentInput, projectId: string): Promise<string | number | undefined> => {
  const queueName = await agentPoolApi.findAgentPoolNameById(agentDeploymentInput.queueId!, projectId)
  return queueName || agentDeploymentInput.queueId
}

const agentDeploymentInputParallelExecution = async (agentDeploymentInput: AgentDeploymentInput): Promise<ExecutionInput | undefined> => {
  const exported = await applyExport(agentDeploymentInput.parallelExecution || {}, exportAgentDeploymentInputParallelExecution)
  return Object.keys(exported).length !== 0 ? exported : undefined
}

const agentDeploymentInputArtifactsDownloadInput = async (agentDeploymentInput: AgentDeploymentInput): Promise<ArtifactsDownloadInput | undefined> => {
  const exported = await applyExport(agentDeploymentInput.artifactsDownloadInput || {}, exportAgentDeploymentInputArtifactsDownloadInput)
  return Object.keys(exported).length !== 0 ? exported : undefined
}

const agentDeploymentInputArtifactsDownloadInputs = async (artifactsDownloadInput: ArtifactsDownloadInput): Promise<ArtifactDownloadInputBase[] | undefined> => {
  return (artifactsDownloadInput.downloadInputs || []).length !== 0 ? artifactsDownloadInput.downloadInputs : undefined
}

const preDeployApprovals = async (environment: ReleaseDefinitionEnvironment): Promise<ReleaseDefinitionApprovals | undefined> => {
  const deployApprovals = await applyExport(environment.preDeployApprovals || {}, exportReleaseDefinitionApprovals)
  return Object.keys(deployApprovals).length !== 0 ? deployApprovals : undefined
}

const preDeploymentGates = async (environment: ReleaseDefinitionEnvironment): Promise<ReleaseDefinitionGatesStep | undefined> => {
  const deploymentGates = await applyExport(environment.preDeploymentGates || {}, exportDeploymentGates)
  return Object.keys(deploymentGates).length !== 0 ? deploymentGates : undefined
}

const gates = async (releaseDefinitionGatesStep: ReleaseDefinitionGatesStep): Promise<ReleaseDefinitionGate[] | undefined> => {
  return (releaseDefinitionGatesStep.gates || []).length !== 0 ? releaseDefinitionGatesStep.gates : undefined
}

const postDeployApprovals = async (environment: ReleaseDefinitionEnvironment): Promise<ReleaseDefinitionApprovals | undefined> => {
  const deployApprovals = await applyExport(environment.postDeployApprovals || {}, exportReleaseDefinitionApprovals)
  return Object.keys(deployApprovals).length !== 0 ? deployApprovals : undefined
}

const postDeploymentGates = async (environment: ReleaseDefinitionEnvironment): Promise<ReleaseDefinitionGatesStep | undefined> => {
  const deploymentGates = await applyExport(environment.postDeploymentGates || {}, exportDeploymentGates)
  return Object.keys(deploymentGates).length !== 0 ? deploymentGates : undefined
}

const approvals = async (releaseDefinitionApprovals: ReleaseDefinitionApprovals): Promise<ReleaseDefinitionApprovalStep[] | undefined> => {
  const exported = (await Promise.all(
    (releaseDefinitionApprovals.approvals || [])
      .map(async releaseDefinitionApproval => {
        return applyExport(releaseDefinitionApproval, exportApproval)
      })
  )).filter(releaseDefinitionApproval => releaseDefinitionApproval && Object.keys(releaseDefinitionApproval).length !== 0)
  return exported.length !== 0 ? exported : undefined
}

const approvalOptions = async (releaseDefinitionApprovals: ReleaseDefinitionApprovals): Promise<ApprovalOptions | undefined> => {
  const exported = await applyExport(releaseDefinitionApprovals.approvalOptions || {}, exportApprovalOptions)
  return Object.keys(exported).length !== 0 ? exported : undefined
}
...
const retentionPolicy = async (environment: ReleaseDefinitionEnvironment): Promise<EnvironmentRetentionPolicy | undefined> => {
  const exported = await applyExport(environment.retentionPolicy || {}, exportRetentionPolicy)
  return Object.keys(exported).length !== 0 ? exported : undefined
}

const schedules = async (environment: ReleaseDefinitionEnvironment): Promise<ReleaseSchedule[] | undefined> => {
  return (environment.schedules || []).length !== 0 ? environment.schedules : undefined
}
...
const exportAgentDeploymentInputArtifactsDownloadInput: ArtifactsDownloadInput | object = {
