...
import { Reporter, TransformedDefinition } from '@kroonprins/pipectl-core/dist/model'
...
abstract class GetReporter<U extends GetProcessResult<W>, V extends TransformedDefinition, W> implements Reporter {
  ...
  canReport(processResult: U, _transformedDefinition: V, _action: Action, args: GetArguments): boolean {
    ...
    log.debug(`[GetReporter] canReport[${result}], processResult[${JSON.stringify(processResult)}], args[${args}]`)
    return result
  }

  async report(processResult: U, transformedDefinition: V, _action: Action, _args: GetArguments): Promise<void> {
    log.debug(`[GetReporter] processResult[${JSON.stringify(processResult)}], transformedDefinition[${JSON.stringify(transformedDefinition)}]`)
