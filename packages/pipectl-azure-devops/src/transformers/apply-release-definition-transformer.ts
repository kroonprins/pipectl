...
class ApplyReleaseDefinitionTransformer extends ReleaseDefinitionTransformer {
  // TODO instead of this, maybe possible to have multiple transformers applied (first ReleaseDefinitionTransformer for all then this one if action === Action.APPLY)?
  ...
  protected async setReleaseDefinitionDefaults(
    ...
    log.debug(
      `[ApplyReleaseDefinitionTransformer.setReleaseDefinitionDefaults] before[${JSON.stringify(
        definition
      )}]`
    )
    const updatedSpec = await applyDefaults(
      await super.setReleaseDefinitionDefaults(definition),
      defaultsReleaseDefinition,
      definition
    )
    log.debug(
      `[ApplyReleaseDefinitionTransformer.setReleaseDefinitionDefaults] after[${JSON.stringify(
        updatedSpec
      )}]`
    )
