...
import { Definition, DefinitionTransformer } from '@kroonprins/pipectl-core/dist/model'
...
class BuildDefinitionTransformer implements DefinitionTransformer {
  ...
  async transform(definition: Definition, _action: Action, _args: CommonArguments) {
    log.debug(`[BuildDefinitionTransformer.transform] definition[${JSON.stringify(definition)}]`)
