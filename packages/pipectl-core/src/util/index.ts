...
const completeDefinitions = (
  ...
  return definitions.map((definition) => {
    if (!definition.apiVersion)
      throw new Error(`Missing apiVersion for definition ${definition}`)
    if (!definition.kind)
      throw new Error(`Missing kind for definition ${definition}`)
    if (!definition.spec)
      throw new Error(`Missing spec for definition ${definition}`)
