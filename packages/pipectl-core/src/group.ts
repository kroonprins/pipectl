...
const group = (
  ...
  return definitions
    .map((definition) => {
      const grouper = groupers().find((g) =>
        g.canGroup(definition, action, args)
      )
      if (!grouper) {
      ...
      log.debug(
        `[group] group[${_group}], knownGroup[${knownGroups}] for ${JSON.stringify(
          definition
        )}`
      )
    ...
    .reduce<DefinitionGroup[]>((acc, val) => {
      log.debug(
        `[group] reducing acc[${JSON.stringify(acc)}], val[${JSON.stringify(
          val
        )}]`
      )
      val.forEach((item, index) => {
        log.debug(
          `[group] definition group item item[${JSON.stringify(
            item
          )}], val[${index}]`
        )
        if (index >= acc.length) {
          ...
          if (!groupAtIndex) {
            log.error('wtf, this should not happen...')
          } else {
            const indexExisting = groupAtIndex.items.findIndex(
              (groupItem) => item.name === groupItem.name
            )
            if (indexExisting !== -1) {
              log.debug(
                '[group] group item exists in accumulator, adding definitions to its items list'
              )
              groupAtIndex.items[indexExisting].definitions.push(
                ...item.definitions
              )
            } else {
              log.debug(
                '[group] group item does not yet exist in accumulator, initializing its items list with definitions'
              )
